a = lambda `123`



a             : source.python
=             : keyword.operator.assignment.python, source.python
              : source.python
lambda        : meta.lambda-function.python, source.python, storage.type.function.lambda.python
              : meta.function.lambda.parameters.python, meta.lambda-function.python, source.python
`             : invalid.illegal.operator.python, meta.function.lambda.parameters.python, meta.lambda-function.python, source.python
123           : meta.function.lambda.parameters.python, meta.lambda-function.python, source.python
`             : invalid.illegal.operator.python, meta.function.lambda.parameters.python, meta.lambda-function.python, source.python
              : meta.lambda-function.python, source.python
